...
const userObjectId = (user) => {
    ...
    return raw ? new mongoose_1.default.Types.ObjectId(String(raw)) : null;
};
...
const getTrip = async (req, res) => {
    try {
        ...
        if (isFieldStaffRole(user.rol) && uid) {
            trips = await Trip_1.default.find(tripAssignedToUserQuery(uid, user.rol)).populate("asignadoPor", "nombre apellido");
        }
        else {
            trips = await Trip_1.default.find().populate("asignadoPor", "nombre apellido");
        }
...
const createTrip = async (req, res) => {
    try {
        const { rutaAcubrir, unidadId, conductorId, fechaSalida, fechaLlegada, destino, estado, kilometrajeSalida, kilometrajeLlegada, acompanante, def, tarjeta, multidestino, destinoExtra, } = req.body;
...
const updateTrip = async (req, res) => {
    try {
        ...
        const { rutaAcubrir, destino, fechaLlegada, fechaSalida, kilometrajeSalida, kilometrajeLlegada, estado, unidadId, conductorId, acompanante, def, tarjeta, multidestino, destinoExtra, destinoActualIndex, } = req.body;
...
const updateTripOperador = async (req, res) => {
    try {
        ...
        if (!trip)
            return res.status(404).json({ message: "Viaje no encontrado" });
        const estadoAnterior = trip.estado;
        const { estado, destinoActualIndex, fechaSalida, fechaLlegada, multidestino, destinoExtra, checklistInicio, checklistRecepcion, checklistFin, checklistParada, } = req.body || {};
        const $set = {};
        ...
        if (checklistParada !== undefined) {
            ...
            if (normalized) {
                ...
                $push.checklistParadas = {
                    ...normalized,
                    index: Number(checklistParada?.index) || 0,
